...
namespace POSInventoryCreditSystem
{
    public partial class AdminAddProducts : UserControl
    {
        ...
        private void InitializeProdIDAutoComplete()
        {
            ...
            {
                prodIDSuggestions.Add(product.ProdID);
            }
        ...
        private void LoadProductDetails(string prodID)
        {
            ...
            // For example:
            var productData = new AddProductsData().AllProductsData().FirstOrDefault(p => p.ProdID == prodID);

            if (productData != null)
            {
                // Populate the fields with product details
                addProducts_prodName.Text = productData.ProdName;
